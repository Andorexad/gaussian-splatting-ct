...
if __name__ == '__main__':
    global_path = 'data/manifest-1600709154662/LIDC-IDRI'
    save_path = "drr"
    # '/media/newuser/AprilsDrive/phd/datasets/xrays/knee/knee_006'
    ...
    for i in range(len(dirs)):
        # print(len(dirs))
        ct_files=[]
        # for j in os.walk(dirs[i]):
        #     print(j)
        for root, _, files in os.walk(dirs[i]):
            dcm_file_paths=[]
            for file in files:
                if file.endswith('.dcm'):
                    full_path = os.path.join(root, file)
                    dcm_file_paths.append(full_path)
            if ct_files == []:
                ct_files = dcm_file_paths
            elif len(dcm_file_paths) > len(ct_files):
                ct_files = dcm_file_paths
    ...
    #        output = qx(set_bone_threshold)
        ...
        if multiple_view_mode:
            ...
            drr_lst = [plasti_path+'/plastimatch', "drr", "-A", "cuda", "-t", "pfm",
